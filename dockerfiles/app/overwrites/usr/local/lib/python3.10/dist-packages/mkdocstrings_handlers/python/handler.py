...
from mkdocstrings.handlers.base import (
    BaseHandler,
    CollectionError,
    CollectorItem,
)
...
class PythonHandler(BaseHandler):
    ...
    def load_inventory(
        ...
        **kwargs: Any,  # noqa: ARG003
        ...
        for item in Inventory.parse_sphinx(
            in_file, domain_filter=domains
        ).values():
            yield item.name, posixpath.join(base_url, item.uri)

    def collect(
        self, identifier: str, config: Mapping[str, Any]
    ) -> CollectorItem:  # noqa: D102
        module_name = identifier.split(".", 1)[0]
        unknown_module = module_name not in self._modules_collection
        if config.get("fallback", False) and unknown_module:
            raise CollectionError(
                "Not loading additional modules during fallback"
            )
        ...
        if unknown_module:
            ...
            try:
                for pre_loaded_module in (
                    final_config.get("preload_modules") or []
                ):
            ...
            if unresolved:
                ...
                logger.debug(
                    f"Unresolved aliases: {', '.join(sorted(unresolved))}"
                )
        ...
        try:
            ...
            raise CollectionError(
                f"{identifier} could not be found"
            ) from error
    ...
    def render(
        self, data: CollectorItem, config: Mapping[str, Any]
    ) -> str:  # noqa: D102 (ignore missing docstring)
        final_config = ChainMap(config, self.default_config)  # type: ignore[arg-type]
    ...
    def update_env(
        self, md: Markdown, config: dict
    ) -> None:  # noqa: D102 (ignore missing docstring)
    ...
    ) -> tuple[str, ...]:  # noqa: D102 (ignore missing docstring)
        anchors = [data.path]
        try:
