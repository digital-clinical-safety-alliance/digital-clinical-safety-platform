    ...
    **options: Any,  # noqa: ARG001
    ...
    while new_offset < len(lines):
        ...
        if line.startswith(4 * " "):
            ...
            if new_offset + 1 < len(lines) and lines[
                new_offset + 1
            ].startswith("---"):
                break
...
def _read_block(
    ...
    while new_offset < len(lines):
        ...
        if (
            is_empty
            and new_offset < len(lines) - 1
            and _is_dash_line(lines[new_offset + 1])
        ):
            break  # Break if a new unnamed section is reached.

        if (
            is_empty
            and new_offset < len(lines) - 2
            and _is_dash_line(lines[new_offset + 2])
        ):
    ...
    **options: Any,
    ...
    for item in items:
        ...
        if choices:
            ...
            match = re.match(
                r"^(?P<annotation>.+),\s+default(?: |: |=)(?P<default>.+)$",
                annotation,
            )
            if match:
        ...
        if annotation is None:
            ...
            for name in names:
                ...
                _warn(
                    docstring,
                    new_offset,
                    f"No types or annotations for parameters {names}",
                )
        else:
            annotation = parse_annotation(
                annotation, docstring, log_level=LogLevel.debug
            )
...
def _read_other_parameters_section(
    ...
    **options: Any,
    ...
    _warn(
        docstring,
        new_offset,
        f"Empty other parameters section at line {offset}",
    )
    return None, new_offset
...
def _read_hazards_section(
    ...
    **options: Any,
    ...
    _warn(
        docstring,
        new_offset,
        f"Empty hazards section at line {offset}",
    )
    return None, new_offset
...
def _read_deprecated_section(
    ...
    **options: Any,
    ...
    if not items:
        _warn(
            docstring, new_offset, f"Empty deprecated section at line {offset}"
        )
        return None, new_offset
...
def _read_returns_section(
    ...
    **options: Any,
    ...
    for index, item in enumerate(items):
        ...
        returns.append(
            DocstringReturn(
                name=name or "", annotation=annotation, description=text
            )
        )
...
def _read_receives_section(
    ...
    **options: Any,
    ...
    if not items:
        _warn(
            docstring, new_offset, f"Empty receives section at line {offset}"
        )
        return None, new_offset
...
def _read_attributes_section(
    ...
    **options: Any,
    ...
    if not items:
        _warn(
            docstring, new_offset, f"Empty attributes section at line {offset}"
        )
        return None, new_offset
    ...
    for item in items:
        ...
        attributes.append(
            DocstringAttribute(
                name=name, annotation=annotation, description=text
            )
        )
    ...
    **options: Any,
    ...
    if not items:
        _warn(
            docstring,
            new_offset,
            f"Empty functions/methods section at line {offset}",
        )
        return None, new_offset
    ...
    for item in items:
        ...
        functions.append(
            DocstringFunction(
                name=name, annotation=signature, description=text
            )
        )
...
def _read_classes_section(
    ...
    **options: Any,
    ...
    for item in items:
        ...
        text = dedent("\n".join(item[1:])).strip()
        classes.append(
            DocstringClass(name=name, annotation=signature, description=text)
        )
...
def _read_modules_section(
    ...
    **options: Any,
    ...
    for item in items:
        ...
        text = dedent("\n".join(item[1:])).strip()
        modules.append(
            DocstringModule(name=name, annotation=signature, description=text)
        )
