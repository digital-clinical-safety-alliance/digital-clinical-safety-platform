...
_section_kind = {
    ...
    "hazards": DocstringSectionKind.hazards,
...
_RE_ADMONITION: Pattern = re.compile(
    r"^(?P<type>[\w][\s\w-]*):(\s+(?P<title>[^\s].*))?\s*$", re.I
)
_RE_NAME_ANNOTATION_DESCRIPTION: Pattern = re.compile(
    r"^(?:(?P<name>\w+)?\s*(?:\((?P<type>.+)\))?:\s*)?(?P<desc>.*)$"
)
...
def _read_block(
    ...
    # loop on next lines
    while new_offset < len(lines) and (
        lines[new_offset].startswith(indent * " ")
        or _is_empty_line(lines[new_offset])
    ):
...
def _read_parameters(
    ...
    **options: Any,
    ...
    for line_number, param_lines in block:
        ...
        description = "\n".join(
            [description.lstrip(), *param_lines[1:]]
        ).rstrip("\n")
        ...
        if annotation is None:
            _warn(
                docstring,
                line_number,
                f"No type or annotation for parameter '{name}'",
            )

        if warn_unknown_params:
            with suppress(
                AttributeError
            ):  # for parameters sections in objects without parameters
...
def _read_parameters_section(
    ...
    **options: Any,
    ...
    parameters, new_offset = _read_parameters(
        docstring, offset=offset, **options
    )
...
def _read_other_parameters_section(
    ...
    **options: Any,
    ...
    parameters, new_offset = _read_parameters(
        docstring, offset=offset, warn_unknown_params=False, **options
    )
    return DocstringSectionOtherParameters(parameters), new_offset


def _read_hazards_section(
    docstring: Docstring,
    *,
    offset: int,
    warn_unknown_params: bool = True,  # noqa: ARG001
    **options: Any,
) -> tuple[DocstringSectionHazards | None, int]:
    parameters, new_offset = _read_parameters(
        docstring, offset=offset, warn_unknown_params=False, **options
    )
    return DocstringSectionHazards(parameters), new_offset
...
def _read_attributes_section(
    ...
    **options: Any,
    ...
    for line_number, attr_lines in block:
        ...
        description = "\n".join(
            [description.lstrip(), *attr_lines[1:]]
        ).rstrip("\n")
...
def _read_functions_section(
    ...
    **options: Any,
    ...
    for line_number, func_lines in block:
        ...
        description = "\n".join(
            [description.lstrip(), *func_lines[1:]]
        ).rstrip("\n")
        ...
        functions.append(
            DocstringFunction(
                name=name, annotation=signature, description=description
            )
        )
...
def _read_classes_section(
    ...
    **options: Any,
    ...
    for line_number, class_lines in block:
        ...
        description = "\n".join(
            [description.lstrip(), *class_lines[1:]]
        ).rstrip("\n")
    ...
    **options: Any,
    ...
    for line_number, module_lines in block:
        try:
            ...
            continue
        description = "\n".join(
            [description.lstrip(), *module_lines[1:]]
        ).rstrip("\n")
    ...
    **options: Any,
    ...
    for line_number, exception_lines in block:
        try:
            ...
            description = "\n".join(
                [description.lstrip(), *exception_lines[1:]]
            ).rstrip("\n")
            # try to compile the annotation to transform it into an expression
            annotation = parse_annotation(annotation, docstring)
            exceptions.append(
                DocstringRaise(annotation=annotation, description=description)
            )
    ...
    **options: Any,
    ...
    for line_number, warning_lines in block:
        try:
            ...
            description = "\n".join(
                [description.lstrip(), *warning_lines[1:]]
            ).rstrip("\n")
            warns.append(
                DocstringWarn(annotation=annotation, description=description)
            )
...
def _read_returns_section(
    ...
    **options: Any,
    ...
    if returns_multiple_items:
        block, new_offset = _read_block_items(
            docstring, offset=offset, **options
        )
    else:
        one_block, new_offset = _read_block(
            docstring, offset=offset, **options
        )
    ...
    for index, (line_number, return_lines) in enumerate(block):
        ...
        description = "\n".join(
            [description.lstrip(), *return_lines[1:]]
        ).rstrip("\n")
        ...
        if annotation:
            ...
            if annotation is None:
                ...
                _warn(
                    docstring,
                    line_number,
                    f"No type or annotation for returned value {returned_value}",
                )

        returns.append(
            DocstringReturn(
                name=name or "", annotation=annotation, description=description
            )
        )
...
def _read_yields_section(
    ...
    **options: Any,
    ...
    for index, (line_number, yield_lines) in enumerate(block):
        ...
        name, annotation, description = match.groups()
        description = "\n".join(
            [description.lstrip(), *yield_lines[1:]]
        ).rstrip("\n")
        ...
        if annotation:
            ...
            if annotation is None:
                ...
                _warn(
                    docstring,
                    line_number,
                    f"No type or annotation for yielded value {yielded_value}",
                )
...
def _read_receives_section(
    ...
    **options: Any,
    ...
    for index, (line_number, receive_lines) in enumerate(block):
        ...
        name, annotation, description = match.groups()
        description = "\n".join(
            [description.lstrip(), *receive_lines[1:]]
        ).rstrip("\n")
        ...
        if annotation is None:
            ...
            _warn(
                docstring,
                line_number,
                f"No type or annotation for received value {received_value}",
            )
...
def _read_examples_section(
    ...
    **options: Any,
    ...
    sub_sections: list[
        tuple[
            Literal[DocstringSectionKind.text, DocstringSectionKind.examples],
            str,
        ]
    ] = []
    ...
    **options: Any,
    ...
    try:
        ...
        _warn(
            docstring,
            new_offset,
            f"Could not parse version, text at line {offset}",
        )
...
_section_reader = {
    ...
    DocstringSectionKind.hazards: _read_hazards_section,
    ...
    **options: Any,
    ...
    while offset < len(lines):
        ...
        if in_code_block:
            ...
            blank_line_above = not has_previous_line or _is_empty_line(
                lines[offset - 1]
            )
            has_next_line = offset < len(lines) - 1
            has_next_lines = offset < len(lines) - 2
            blank_line_below = has_next_line and _is_empty_line(
                lines[offset + 1]
            )
            blank_lines_below = has_next_lines and _is_empty_line(
                lines[offset + 2]
            )
            indented_line_below = (
                has_next_line
                and not blank_line_below
                and lines[offset + 1].startswith(" ")
            )
            indented_lines_below = (
                has_next_lines
                and not blank_lines_below
                and lines[offset + 2].startswith(" ")
            )
            ...
            if (
                indented_line_below or indented_lines_below
            ) and not blank_line_above:
            ...
            if is_section:
                if current_section:
                    ...
                    current_section = []
                reader = _section_reader[
                    _section_kind[admonition_type.lower()]
                ]
                ...
                if contents:
                    ...
                    sections.append(
                        DocstringSectionAdmonition(
                            kind=admonition_type, text=contents, title=title
                        )
                    )
                else:
