...
class BuilderTestDocsEmpty(TestCase):
    ...
    def test_copy_templates(self):
        ...
        self.assertEqual(
            d.FILES_EXPECTED_ON_TEMPLATES_COPY.sort(),
            files_to_check.sort(),
        )
    ...
    def test_empty_docs_folder(self):
        open(
            c.TESTING_MKDOCS_PLACEHOLDERS_YAML,
            "a",
        ).close()
        doc_build = Builder(c.TESTING_MKDOCS)
        doc_build.empty_docs_folder()
        self.assertEqual(
            os.listdir(c.TESTING_MKDOCS_DOCS),
            [".gitkeep"],
        )
...
class BuilderTestDocsPresent(TestCase):
    ...
    def test_get_placeholders_file_empty(self):
        ...
        open(
            f"{ c.TESTING_MKDOCS }docs/placeholders.yml",
            "w",
        ).close()
    ...
    def test_get_placeholders_only_one_placeholder_in_yaml(
        self,
    ):
        doc_build = Builder(c.TESTING_MKDOCS)
        self.assertEqual(
    ...
    def test_get_placeholders_empty_docs_folder(
        self,
    ):
        doc_build = Builder(c.TESTING_MKDOCS_EMPTY_FOLDERS)
        with self.assertRaises(FileNotFoundError) as error:
