...
class GitControllerTest(TestCase):
    ...
    def test_init_env_location_empty(self):
        ...
        self.assertEqual(
            str(error.exception),
            f".env location is set to empty string",
        )
    ...
    def test_init_email_bad(self):
        ...
        self.assertEqual(
            str(error.exception),
            f"Email address '{ d.EMAIL_BAD }' is invalid",
        )
    ...
    def test_check_github_credentials(self, mock_github, mock_get):
        ...
        self.assertEqual(
            calls_github[0].args,
            d.CHECK_CREDENTIALS_GITHUB_CALL,
        )
    ...
    def test_check_github_credentials_no_connection(
        ...
        self.assertEqual(
            calls[0],
            d.CHECK_CREDENTIALS_GET_CALLS[0],
        )
    ...
    def test_check_github_credentials_username_bad(
        ...
        self.assertEqual(
            gc.check_github_credentials(),
            d.CREDENTIALS_CHECK_USERNAME_BAD,
        )
    ...
    def test_check_github_credentials_organisation_bad(
        ...
        self.assertEqual(
            gc.check_github_credentials(),
            d.CREDENTIALS_CHECK_ORGANISATION_BAD,
        )
    ...
    def test_organisation_exists(self, mock_get):
        ...
        self.assertEqual(
            calls[0],
            d.CHECK_CREDENTIALS_GET_CALLS[1],
        )
    ...
    def test_commit_and_push_already_committed(
        self,
    ):
        pass
    ...
    def test_entry_new(self):
        ...
        gc.entry_new("title", "body", ["hazard"])
        # TODO - should really check hazard was created
        close_all_issues()

    @tag("git")
    def test_entry_new_label_bad(self):
        gc = GitController(
            github_repo=d.REPO_NAME_CURRENT,
            env_location=c.TESTING_ENV_PATH_GIT,
        )
    ...
    @tag("git")
    def test_entry_new_repo_bad(self):
        gc = GitController(
    ...
    def test_available_hazard_labels_full(self):
        ...
        self.assertEqual(
            ...
            d.AVAILABLE_HAZARD_LABELS_FULL,
        )

    @tag("git")
    def test_available_hazard_labels_name_only(
        self,
    ):
        gc = GitController(env_location=c.TESTING_ENV_PATH_GIT)
        self.assertEqual(
    ...
    @tag("git")
    def test_available_hazard_labels_details_wrong(
        self,
    ):
        gc = GitController(env_location=c.TESTING_ENV_PATH_GIT)
        with self.assertRaises(ValueError) as error:
    ...
    def test_available_hazard_labels_yaml_missing(
        self,
    ):
    ...
    def test_add_comment_to_hazard_number_missing(
        self,
    ):
        gc = GitController(
            github_repo=d.REPO_NAME_CURRENT,
        ...
        self.assertEqual(
            str(error.exception),
            "No Hazard Number has been provided",
        )

    def test_add_comment_to_hazard_comment_missing(
        self,
    ):
        gc = GitController(
            github_repo=d.REPO_NAME_CURRENT,
